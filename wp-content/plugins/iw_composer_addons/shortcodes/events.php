...
if ( ! class_exists( 'Inwave_Events' ) ) {
	...
	class Inwave_Events {
		...
		function heading_init() {
			...
			vc_map( array(
				'name'        => 'Events',
				'description' => __( 'Display a list of events', 'inwavethemes' ),
				'base'        => 'inwave_events',
				...
				'params'      => array(
					array(
						...
						"param_name"  => "title",
						"description" => __( 'Title of events block.', "inwavethemes" )
					),
					array(
						'type'        => 'textfield',
						"holder"      => "div",
						"heading"     => __( "Sup Title", "inwavethemes" ),
						"param_name"  => "sub_title",
						"description" => __( 'Sub Title of events block.', "inwavethemes" )
					),
					array(
						'type'        => 'textarea',
						"holder"      => "div",
						...
						"description" => __( 'Description of events block.', "inwavethemes" )
					),
					array(
						'type'        => 'textfield',
						"heading"     => __( "Number of months", "inwavethemes" ),
						"value"       => "5",
						"param_name"  => "months",
						"description" => __( 'Number of months to get events.', "inwavethemes" )
					),
					array(
						"type"        => "textfield",
						"heading"     => __( "Events in month", "inwavethemes" ),
						"param_name"  => "events",
						"value"       => "5",
						"description" => __( 'Number of events to display on every month. (For Style 2 - Flat list only)', "inwavethemes" )
					),
					array(
						"type"        => "textfield",
						"heading"     => __( "Events in day", "inwavethemes" ),
						"param_name"  => "day_events",
						"value"       => "1",
						"description" => __( 'Number of events to display on every day. (For Timetable 3 - Flat list only)', "inwavethemes" )
					),
					array(
						"type"       => "textfield",
						"heading"    => __( "Month increment", "inwavethemes" ),
						"param_name" => "month_increment",
						"value"      => "",
					),
					array(
						"type"       => "textfield",
						"heading"    => __( "Fixed month", "inwavethemes" ),
						"param_name" => "fixed_month",
						"value"      => "",
					),
					array(
						"type"       => "textfield",
						"heading"    => __( "Fixed year", "inwavethemes" ),
						"param_name" => "fixed_year",
						"value"      => "",
					),
					array(
						"type"       => "dropdown",
						"heading"    => __( "Event order", "inwavethemes" ),
					...
					array(
						"type"       => "dropdown",
						"heading"    => __( "First day of week", "inwavethemes" ),
					...
					array(
						"type"       => "dropdown",
						"heading"    => __( "Hide Past Events", "inwavethemes" ),
					...
					array(
						"type"       => "dropdown",
						"heading"    => __( "Show only featured events", "inwavethemes" ),
		...
		function inwave_events_shortcode( $atts, $content = null ) {
			...
			return $this->htmlBoxEventRender(
				$title, $sub_title, $description, $months, $events, $day_events, $month_increment, $fixed_month, $fixed_year, $event_order, $hide_past, $show_feature_event_only, $class, $style, $first_day_of_week
			);
		}
		...
		function htmlBoxEventRender(
			...
			$get_new_monthyear      = array(
				'month' => $fixed_month ? $fixed_month : date( 'n' ),
				'year'  => $fixed_year ? $fixed_year : date( 'Y' )
			);
			...
			if ( $hide_past == 'yes' ) {
				$h_events = array();
				foreach ( $event_list_array as $event ) {
					if ( $event['event_end_unix'] >= ( time() + get_option( 'gmt_offset' ) * 3600 ) ) {
			...
			if ( $style == 'style1' ) {
				...
				$html = $this->loadTimetableStyleHtml( $event_list_array, $events, $day_events, $class, $title, $sub_title, $description, $months, $first_day_of_week );
			} else {
				$html = 'No style';
			}
										...
										<div class="event-month">
											<div class="table-cell">
												<div class="title">
													...
													<p><?php echo strftime( '%B', $value[0]['event_start_unix'] ); ?></p>
												</div>
												<div class="next">
											...
											foreach ( $value as $event ):
												...
												<div class="sidebar-bottom-1 col-md-12 col-sm-12 col-xs-12">
													...
													<div class="title-match col-md-12 col-sm-6 col-xs-12">
														<h3>
															<a href="<?php echo get_permalink( $event['event_id'] ); ?>" alt=""><?php echo $event['event_title'] ?></a>
														</h3>
														<?php
														$sday = date( 'd', $event['event_start_unix'] );
														$eday = date( 'd', $event['event_end_unix'] );
														if ( $sday == $eday ) {
															$s_day = date( get_option( 'time_format' ), $event['event_start_unix'] );
															$e_day = date( get_option( 'time_format' ), $event['event_end_unix'] );
														} else {
															$s_day = strftime( '%b %d', $event['event_start_unix'] );
															$e_day = strftime( '%b %d', $event['event_end_unix'] );
														}
														?>
								...
								<div class="news-content">
									<ul>
										<?php for ( $i = 0; $i < $events; $i ++ ): ?>
											...
											if ( $eventList[ $i ] ):
												$event = $eventList[ $i ];
												$day = strftime( '%d', $event['event_start_unix'] );
												$month = strftime( '%b', $event['event_start_unix'] );
		...
		public function loadTimetableStyleHtml( $eventList, $events, $day_events, $class, $title, $sub_title, $description, $num_month, $first_day_of_week ) {
										...
										foreach ( $weekEvents as $weekEvent ):
											...
											<div class="time-table-main <?php echo $active ?>">
												<div class="octember col-md-3 col-sm-6 col-xs-12">
													<div class="month">
														<div class="timetable-title">
															...
															<p><?php echo strftime( '%B', strtotime( $dateFirst['date'] ) ); ?></p>
														</div>
														<div class="next">
															<button class="circle icon-wrap t-nav-prev">
												...
												foreach ( $weekEvent as $days ):
													...
													$month = strftime( '%B', strtotime( $days['date'] ) );
													$dayOfWeek = strftime( '%A', strtotime( $days['date'] ) );
													...
													<div class="monday day col-md-3 col-sm-6 col-xs-12">
														<div class="timetable-cont">
															...
															if ( count( $days['event'] ) ):
																...
																<div class="box-content <?php if ( $day_events > 1 ) {
																	echo 'box-content-days';
																} ?>">
																	<div class="table">
																		<div class="box-cell">
																			<div class="timetable-info">
																				<?php
																				for ( $i = 0; $i < count( $days['event'] ) && $i < $day_events; $i ++ ) {
																					$event       = $days['event'][ $i ];
																					$link        = get_permalink( $event['event_id'] );
																					$event_title = $event['event_title'];
																					$sday        = date( 'd', $event['event_start_unix'] );
																					...
																					<?php if ( $day_events == 1 ) {
																						...
																						<div class="timetable_details">
																							<a href="<?php echo $link; ?>"><?php _e( 'Read more', 'inwavethemes' ); ?></a>
																						</div>
																						<?php
																					}
																				}
																...
																<div class="box-content">
																	<div class="table">
																		<?php if ( ! $isDisable ): ?>
																			<div class="box-cell">
																				<div class="timetable-info">
																					<span><?php _e( 'No event', 'inwavethemes' ); ?></span>
																				</div>
																			</div>
																		<?php endif; ?>
																	</div>
		...
		public function weekEvents( $eventList, $num_month, $first_day_of_week ) {
			...
			$listWeeks    = array();
			if ( $numberEvents > 0 ) {
				$sdate = strtotime( date( 'Y-m-01', $eventList[0]['event_start_unix'] ) );
				...
				for ( $i = 0; $i < $days; $i ++ ) {
					...
					foreach ( $eventList as $event ) {
						if ( date( 'Y-m-d', $event['event_start_unix'] ) == $date ) {
							if ( key_exists( $i, $listWeeks ) ) {
								$listWeeks[ $i ]['event'][] = $event;
							} else {
								$listWeeks[ $i ] = array( 'date'      => $date,
								                          'isDisable' => $isDisable,
								                          'event'     => array( $event )
								);
							}
