...
class coursesBox {
    ...
    public function add_meta_box($post_type) {
        ...
        if ($post_type == 'iw_courses') {
            add_meta_box(
                    'course_image_gallery', __('Image Gallery', 'inwavethemes'), array($this, 'render_meta_box_image_gallery'), $post_type, 'advanced', 'high'
            );
            add_meta_box(
                    'course_advance_relative', __('Advanced', 'inwavethemes'), array($this, 'render_meta_box_advanced'), $post_type, 'advanced', 'high'
            );
            add_meta_box(
                    'course_extra_fields', __('Extra Fields', 'inwavethemes'), array($this, 'render_meta_box_extra_fields'), $post_type, 'advanced', 'high'
            );
        }
    }
    ...
    public function save($post_id) {
        ...
        $image_gallery = array();
        if (isset($iw_information['image_gallery']) && $iw_information['image_gallery']) {
            foreach ($iw_information['image_gallery'] as $value) {
                $image_gallery[] = str_replace(site_url(), '', $value);
            }
        }
        $related_couses = array();
        if (isset($iw_information['related_couses']) && $iw_information['related_couses']) {
        ...
        update_post_meta($post_id, 'iw_courses_price', $iw_information['price']);
        ...
        <div class="iwc-metabox-fields">
            <table class="list-table">
                <tbody class="the-list">
                    <tr class="alternate">
                        <td>
                            <label for='teacher_relative'><strong><?php echo __('Teacher', 'inwavethemes'); ?></strong></label>
                        </td>
                        <td>
                            <?php
                            ...
                            echo $utinity->selectFieldRender('teacher_relative', 'iw_information[teacher]', $teacher_value, $data, 'Select Teacher', '');
                            ?>
                        </td>
                    </tr>
                    <tr class="alternate">
                        <td>
                            <label><?php echo sprintf(__('Price (%s)', 'inwavethemes'), $utinity->getIWCurrencySymbol($utinity->getCoursesOption('currency', 'USD'))); ?></label>
                        </td>
                        <td>
                            <input class="input-field" name="iw_information[price]" type="number" min="0" required="" value="<?php echo get_post_meta($post->ID, 'iw_courses_price', true) ?>"/>
                        </td>
                    </tr>
                </tbody>
            </table>
            <h3><?php echo __('Related Courses: ', 'inwavethemes'); ?></h3>
            <?php
            ...
            <table class="list-table">
                <thead>
                    <tr>
                        <th class="left"><?php echo __('ID', 'inwavethemes'); ?></th>
                        <th><?php echo __('Title', 'inwavethemes'); ?></th>
                        <th></th>
                    </tr>
                </thead>
                <tbody class="the-list">
                    ...
                    <tr>
                        ...
                        <td>
                            <span style="width: 100%;" class="button add-row courses-related"><?php echo __('Add Courses', 'inwavethemes'); ?></span
                        </td>
                    </tr>
                </tbody>
            </table>
                                        ...
                                        case 'dropdown_list':
                                            ...
                                            foreach ($options as $option) {
                                                ...
                                                if (count($option_data) == 3) {
                                                    ...
                                                    if ($meta) {
                                                        $values = maybe_unserialize(html_entity_decode($meta));
                                                        if(!is_array($values)){
                                                            $values = array($values);
                                                        }
                                                        $val = $option_data[0];
                                                        if ($values && in_array($val, $values)) {
