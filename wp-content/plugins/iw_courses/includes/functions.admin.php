...
// Register Custom Post Type
    function iw_courses_add_post_types_courses() {
        ...
        if (get_option('iw_courses_settings')['enable_voting'] && get_option('iw_courses_settings')['enable_voting'] == 1) {
...
if (!function_exists('adminAddScript')) {
    ...
    function adminAddScript() {
        ...
        if (substr_count($screen->id,'iw_courses')){
...
//
            if (isset($_POST['date_value']) && $_POST['date_value'] != '') {
...
//
            ...
            $_SESSION['extrafield']['published'] = $published;
//
            ...
            if ($msg == '') {
                ...
                if (isset($_POST['id']) && $_POST['id']) {
                    ...
                    $_SESSION['bt_message'] = $utility->getMessage(__('Extrafield has been updated'));
                    ...
                    if ($ins) {
                        ...
                        $_SESSION['bt_message'] = $utility->getMessage(__('Extrafield has been created'));
                        unset($_SESSION['extrafield']);
...
if(!function_exists('iwcUpdateOrdering')){
    function iwcUpdateOrdering(){
        global $wpdb;
        $id = filter_input(INPUT_POST, 'id');
        $value = filter_input(INPUT_POST, 'value');
        $wpdb->update($wpdb->prefix.'iw_courses_extrafields',array('ordering'=>$value), array('id'=>$id));
        echo 1;
        exit;
    }
}

function course_product_woocommerce_data_stores( $stores ) {

    require_once plugin_dir_path( __DIR__ ) . 'includes/classes/course_product.php';
    $stores['product'] = 'Course_Product_Data_Store_CPT';

    return $stores;
}

function course_product_get_price($price, $product){
	if(get_post_type($product->get_id()) === 'iw_courses'){
		$price = (get_post_meta($product->get_id(), 'iw_courses_price', true)?get_post_meta($product->get_id(), 'iw_courses_price', true):0);
	}
	return $price;
}