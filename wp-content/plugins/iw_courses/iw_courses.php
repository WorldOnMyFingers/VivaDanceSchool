...
/*
  ...
  Plugin URI: http://courses.inwavethemes.com
...
function iwcAddAdminMenu() {
    add_options_page(__('IWC Settings', 'inwavethemes'), __('IWC Settings', 'inwavethemes'), 'manage_options', 'iw_courses_settings', 'optionRenderPage');
    add_submenu_page('edit.php?post_type=iw_courses', __('Extra Fields', 'inwavethemes'), __('Extra Fields', 'inwavethemes'), 'manage_options', 'extra-field', 'iwcExtrafieldRenderPage');
    add_submenu_page(null, null, __('Add Extra Field', 'inwavethemes'), 'manage_options', 'add-extra-field', 'iwcAddExtrafieldRenderPage');
    add_submenu_page('edit.php?post_type=iw_courses', __('Teachers', 'inwavethemes'), __('Teachers', 'inwavethemes'), 'manage_options', 'edit.php?post_type=iw_teacher');
    add_submenu_page('edit.php?post_type=iw_courses', __('IWC Settings', 'inwavethemes'), __('Settings', 'inwavethemes'), 'manage_options', 'settings', 'optionRenderPage');
}
...
function iw_courses_list__teacher_shortcode($atts) {
    ...
    // Code
    return iwcTeacherListHtmlPage($theme, $item_per_page, $show_paging, $page);
}
...
//add ajax action iwcLoadCoursesPosts
add_action('wp_ajax_nopriv_iwcUpdateOrdering', 'iwcUpdateOrdering');
add_action('wp_ajax_iwcUpdateOrdering', 'iwcUpdateOrdering');

$utility = new iwcUtility();
if($utility->checkUseWoocommercePayment()){
	add_filter( 'woocommerce_data_stores', 'course_product_woocommerce_data_stores' );
	add_filter('woocommerce_product_get_price', 'course_product_get_price', 10, 2 );
}
