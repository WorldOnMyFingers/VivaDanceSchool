 ...
 * @version 1.0.0
 */
require_once WP_PLUGIN_DIR . '/ap_background/includes/utility.php';
require_once WP_PLUGIN_DIR . '/ap_background/includes/images.php';
...
if (!function_exists('pluginIndexRenderPage')) {
    ...
    function pluginIndexRenderPage() {
        include_once WP_PLUGIN_DIR . '/ap_background/includes/html/index.php';
    }
...
if (!function_exists('pluginAddnewRenderPage')) {
    ...
    function pluginAddnewRenderPage() {
        include_once WP_PLUGIN_DIR . '/ap_background/includes/html/create_new.php';
    }
...
if (!function_exists('pluginPreviewPage')) {
    ...
    function pluginPreviewPage() {
        include_once WP_PLUGIN_DIR . '/ap_background/includes/html/preview.php';
    }
...
//                    Khoi tao duong dan va thu muc chua anh
                ...
                if (isset($_POST['ap_background_facebook_token'])) {
                    if (!get_option('ap_background_facebook_token')) {
                        add_option('ap_background_facebook_token', $_POST['ap_background_facebook_token']);
                    } else {
                        update_option('ap_background_facebook_token', $_POST['ap_background_facebook_token']);
                    }
                }
...
function getImageAlbums() {
    ...
    if ($source == 'facebook') {
        ...
        if (isset($list_album) && $list_album) {
            ...
            $result['success'] = TRUE;
            $result['message'] = __('Error validating access token: The session was invalidated explicitly using an API call.', 'inwavethemes');
            $result['data'] = '<option value="">' . __('Please select an album') . '</option>';
        }
    }
        ...
        if (filter_var($url, FILTER_VALIDATE_URL)) {
            ...
            if (count($parse_url_1) > 1) {
                ...
                foreach ($parse_url as $value) {//Truong hop url nhap vao la video
                    ...
                    if ($param[0] == "list") {
                        if (!$api) {
                            $result['message'] = __('Please input google api', 'inwavethemes');
                        } else {
                            $playlist_id = $param[1];
                            $result['success'] = TRUE;
...
function listCopyItem() {
    ...
    if ($ins) {
        ...
        $html[] = '<a href="' . admin_url('admin.php?page=bt-advance-parallax-background/create-new&content_type=' . $settings->content_type . '&id=' . $idNew) . '">';
        $html[] = '<span class="button green" title="' . __('Edit', 'inwavethemes') . '"><i class="fa fa-pencil"></i></span>';
        $html[] = '</a>';
        $html[] = '<span class="button red delete" title="' . __('Delete', 'inwavethemes') . '"><i class="fa fa-times"></i></span>';
        $html[] = '<span class="button blue copy" title="' . __('Copy', 'inwavethemes') . '"><i class="fa fa-copy"></i></span>';
        $html[] = '<a href="' . admin_url('admin.php?page=bt-advance-parallax-background/preview&id=' . $idNew) . '" target="_blank">';
        $html[] = '<span class="button original preview" title="' . __('Preview', 'inwavethemes') . '"><i class="fa fa-desktop"></i></span>';
        $html[] = '</a>';
...
function addVisualComposerButton() {
    ...
    vc_map(array(
        "name" => __("AP Background", 'inwavethemes'),
        "description" => __("BT Advanced Parallax Background", 'inwavethemes'),
        ...
        "params" => array(
            array(
                ...
                "holder" => "div",
                "class" => "",
                "heading" => __("Select Background", 'inwavethemes'),
                "param_name" => "alias",
                "value" => btParallaxBackgroundUtility::getParallaxListOptions(true),
                "description" => __("Choose BT Advenced Parallax Background", 'inwavethemes')
            )
        )
    ));
}
