...
function adv_parallax_add_site_script() {
    ...
    wp_enqueue_style('custombox', plugins_url('ap_background/assets/css/custombox.min.css'));
    ...
    wp_enqueue_script('custombox', plugins_url('ap_background/assets/js/custombox.min.js'), array('jquery'), '1.0.0', true);
...
function loadParallaxFrameContent() {
    ...
    if (!$item) {//Neu khong lay duoc thong tin cua parallax thi hien thi text thay the
        $result['message'] = __('No Parallax Background with alias: ').'<strong>' . $alias . '</strong>';
        ...
        if ($item->settings->content_type == 'image-gallery'):
            include_once WP_PLUGIN_DIR . '/ap_background/includes/html/parallax_content/image.php';
        endif;

        //Xu ly load noi dung voi truong hop kieu noi dung video
        if ($item->settings->content_type == 'video-background'):
            include_once WP_PLUGIN_DIR . '/ap_background/includes/html/parallax_content/video.php';
        endif;

        //Xu ly load noi dung voi truong hop kieu noi dung woo commerce
        if ($item->settings->content_type == 'woo-commerce'):
            include_once WP_PLUGIN_DIR . '/ap_background/includes/html/parallax_content/woo-commerce.php';
        endif;

        //Xu ly load noi dung voi truong hop kieu noi dung wp post
        if ($item->settings->content_type == 'wordpress-posts'):
            include_once WP_PLUGIN_DIR . '/ap_background/includes/html/parallax_content/wp-post.php';
        endif;
