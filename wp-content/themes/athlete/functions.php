...
if (!function_exists('athlete_comment')) {
    ...
    if (!function_exists('getSocialSharing')) {
        ...
        function getSocialSharing($link, $text, $title, $tag='') {
            ...
            if ($smof_data['sharing_vkontakte']) {
                $shareLink = 'https://vk.com/share.php?url=' . $link . '&amp;description=' . $text . '&amp;title=' . $title;
                echo ($tag?'<'.$tag.'>':'').'<a target="_blank" href="'.esc_url($shareLink).'" title="' . esc_attr_x('VKontakte', 'inwavethemes') . '" '.$newWindow.'><i class="fa fa-vk"></i></a>'.($tag?'</'.$tag.'>':'');
            }
